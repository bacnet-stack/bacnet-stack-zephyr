			     ...
			     uint32_t array_index, float default_value, float *value)
{
	...
	stored_len = bacnet_settings_value_get(object_type, object_instance, property_id,
					       array_index, &bvalue);
			      ...
			      uint32_t array_index, float value)
{
	...
	return bacnet_settings_value_set(object_type, object_instance, property_id, array_index,
					 &bvalue);
}
					...
					const char *default_value, BACNET_CHARACTER_STRING *value)
{
	...
	stored_len = bacnet_storage_get(&key, name, sizeof(name));
	if (stored_len > 0) {
		len = bacnet_character_string_application_decode(name, stored_len, value);
		if (len <= 0) {
					      ...
					      const char *cstring)
{
	...
	if (!status) {
		status = bacnet_settings_value_set(object_type, object_instance, property_id,
						   array_index, &bvalue);
	}
