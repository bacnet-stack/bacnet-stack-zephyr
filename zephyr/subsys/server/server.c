...
K_MSGQ_DEFINE(
    bacnet_server_msgq,
    sizeof(struct bacnet_server_msg),
    8,
 ...
 */
static void ipv4_events_handler(
    struct net_mgmt_event_callback *cb, u32_t mgmt_event, struct net_if *iface)
{
    ...
    if (ret != 0) {
        printk(
            "\n*** Handler[%d]: queue full, type %u event 0x%08x on iface %p "
            "dropped! ***\n",
...
static void server_thread(void)
{
    ...
    net_mgmt_init_event_callback(
        &mgmt_cb, ipv4_events_handler, SERVER_IPV4_EVENTS_MASK);
    ...
    while (1) {
        ...
        if (-EAGAIN == ret) {
            ...
            uint16_t pdu_len =
                bip_receive(&src, &BIP_Rx_Buffer[0], MAX_MPDU, 5);

            /* process */
            if (pdu_len) {
                LOG_INF("Server: BIP received %u bytes.", (unsigned)pdu_len);
            }
        }
        ...
        /* Message received */
            ...
            switch (msg.type) {
                ...
                case SERVER_MSG_TYPE_IPV4_EVENT: {
                    LOG_INF(
                        "Server: MSG_TYPE_IPV4_EVENT u32: %08x ptr: %p",
...
#endif
                        default:
                            LOG_INF("Server: Unsupported event %u", mgmt_event);
                            break;
                    }
                ...
                default:
                    LOG_WRN("Server: Dropping unsupported type %u", msg.type);
                    break;
            }
        }
        ...
        /* Returned without waiting and without message - why? */
            ...
            LOG_WRN("Server: Msgq returned w/o timeout or msg! req = %d", ret);
        }
    }
}
...
static int server_init(struct device *dev)
{
    ...
    k_thread_create(
        &server_thread_data, server_thread_stack,
        K_THREAD_STACK_SIZEOF(server_thread_stack),
        (k_thread_entry_t)server_thread, NULL, NULL, NULL,
        K_PRIO_PREEMPT(CONFIG_BACNETSTACK_BACNET_SERVER_PRIO), 0, K_NO_WAIT);
...
SYS_INIT(
    server_init, APPLICATION, CONFIG_BACNETSTACK_BACNET_SERVER_APP_PRIORITY);
