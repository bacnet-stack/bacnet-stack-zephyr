...
void bip6_set_interface(char *ifname)
{
    ...
    if (interface) {
        ...
        LOG_INF(
            "BIP6: Using configured index %d", CONFIG_BACDL_BIP6_ADDRESS_INDEX);
        memcpy(
            &unicast.address,
...
static int bvlc6_snprintf_addr(char *s, size_t n, BACNET_IP6_ADDRESS *addr)
{
    ...
    return snprintf(
        s, n, "%04X:%04X:%04X:%04X:%04X:%04X:%04X:%04X", addr16[0], addr16[1],
...
bool bip6_init(char *ifname)
{
    ...
    status = zsock_bind(
        BIP6_Socket, (const struct sockaddr *)&server, sizeof(server));
    if (status < 0) {
        zsock_close(BIP6_Socket);
        BIP6_Socket = -1;
