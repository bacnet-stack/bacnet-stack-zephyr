...
static char *inet6_ntoa(struct in6_addr *a)
{
	...
	for (x = 0; x < IP6_ADDRESS_MAX; x += 2) {
		if (a->s6_addr[x] | a->s6_addr[x + 1]) {
			non_zero_count++;
			d += snprintf(&ipv6_addr_str[d], sizeof(ipv6_addr_str), "%02X%02X",
...
void bip6_set_interface(char *ifname)
{
	...
	if (ifname) {
		...
		if (!index && strcmp(ifname, "0")) {
			...
			if (interface) {
				...
				LOG_ERR("%s:%d - No interface at index %d", THIS_FILE, __LINE__,
					index);
			}
		}
	}
	...
	if (interface) {
		...
		LOG_INF("BIP6: Using configured index %d", CONFIG_BACDL_BIP6_ADDRESS_INDEX);
		memcpy(&unicast.address,
		...
		if (net_addr_pton(AF_INET6, CONFIG_BACDL_BIP6_MCAST_ADDRESS, &multicast.address)) {
			LOG_ERR("%s:%d - Failed to parse IPv6 multicast address: %s", THIS_FILE,
...
int bip6_send_mpdu(BACNET_IP6_ADDRESS *dest, uint8_t *mtu, uint16_t mtu_len)
{
	...
	bvlc6_address_get(dest, &addr16[0], &addr16[1], &addr16[2], &addr16[3], &addr16[4],
	...
	/* Send the packet */
	return zsock_sendto(BIP6_Socket, (char *)mtu, mtu_len, 0, (struct sockaddr *)&bvlc_dest,
			    sizeof(bvlc_dest));
}
...
static int bvlc6_snprintf_addr(char *s, size_t n, BACNET_IP6_ADDRESS *addr)
{
	...
	bvlc6_address_get(addr, &addr16[0], &addr16[1], &addr16[2], &addr16[3], &addr16[4],
			  &addr16[5], &addr16[6], &addr16[7]);

	return snprintf(s, n, "%04X:%04X:%04X:%04X:%04X:%04X:%04X:%04X", addr16[0], addr16[1],
			addr16[2], addr16[3], addr16[4], addr16[5], addr16[6], addr16[7]);
}
 ...
 */
uint16_t bip6_receive(BACNET_ADDRESS *src, uint8_t *npdu, uint16_t max_npdu, unsigned timeout)
{
	...
	if (zsock_select(max + 1, &read_fds, NULL, NULL, &select_timeout) > 0) {
		received_bytes = zsock_recvfrom(BIP6_Socket, (char *)&npdu[0], max_npdu, 0,
...
bool bip6_init(char *ifname)
{
	...
	if (BIP6_Broadcast_Addr.address[0] == 0) {
		bvlc6_address_set(&BIP6_Broadcast_Addr, BIP6_MULTICAST_SITE_LOCAL, 0, 0, 0, 0, 0, 0,
	...
	status = zsock_setsockopt(BIP6_Socket, SOL_SOCKET, SO_REUSEADDR, &sockopt, sizeof(sockopt));
	if (status < 0) {
		LOG_ERR("BIP6: setsockopt(SO_REUSEADDR)");
	}
	/* allow us to send a broadcast */
	status = zsock_setsockopt(BIP6_Socket, SOL_SOCKET, SO_BROADCAST, &sockopt, sizeof(sockopt));
	if (status < 0) {
	...
	status = setsockopt(BIP6_Socket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &join_request,
			    sizeof(join_request));
	if (status < 0) {
	...
	status = zsock_bind(BIP6_Socket, (const struct sockaddr *)&server, sizeof(server));
	if (status < 0) {
		zsock_close(BIP6_Socket);
		BIP6_Socket = -1;
